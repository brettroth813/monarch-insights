...
class SupplementStore:
    ...
    # Allowlist of tables + columns the dynamic ``upsert`` helper is willing to write.
    # Values come from trusted internal callers today, but we validate the names on
    # every call so a future caller can't slip SQL injection in via dict keys.
    _ALLOWED_COLUMNS: dict[str, set[str]] = {
        "cost_basis_lots": {
            "id", "account_id", "ticker", "quantity", "acquired_on",
            "cost_per_share", "fees", "source", "notes", "created_at", "updated_at",
        },
        "paystubs": {
            "id", "employer", "period_start", "period_end", "paid_on",
            "gross_pay", "net_pay", "detail_json", "document_id", "transaction_id",
            "created_at",
        },
        "paystub_line_items": {
            "id", "paystub_id", "category", "label", "amount", "ytd_amount", "pretax",
        },
        "income_sources": {
            "id", "name", "source_type", "employer_or_payer", "notes", "is_active",
        },
        "income_events": {
            "id", "source_id", "on_date", "gross_amount", "taxable_amount",
            "withholding_amount", "detail_json", "transaction_id",
        },
        "rsu_grants": {
            "id", "employer", "grant_date", "shares", "vest_schedule_json",
            "fmv_at_grant", "notes",
        },
        "allocation_targets": {
            "id", "bucket", "target_pct", "drift_threshold_pct", "notes",
        },
        "financial_plans": {
            "id", "name", "detail_json", "is_active", "created_at",
        },
        "documents": {
            "id", "title", "doc_type", "tax_year", "institution", "storage_kind",
            "storage_ref", "sha256", "notes", "metadata_json", "added_at",
        },
        "info_requests": {
            "id", "kind", "summary", "detail_json", "status", "severity",
            "suggested_action", "related_account_id", "related_ticker",
            "created_at", "resolved_at",
        },
        "notes": {
            "id", "subject", "body", "related_id", "related_kind", "created_at",
        },
    }
    ...
    def upsert(self, table: str, row: dict[str, Any], key: str = "id") -> None:
        ...
        Builds SQL from Python strings, but refuses any table or column not in
        :attr:`_ALLOWED_COLUMNS`. Callers that pass untrusted input therefore get a
        :class:`ValueError` instead of a successful injection.

        Args:
            table: Table name. Must be a key of ``_ALLOWED_COLUMNS``.
            row: ``{column_name: value}`` payload.
            key: Column name used for the ``ON CONFLICT`` clause.
        """
        allowed = self._ALLOWED_COLUMNS.get(table)
        if allowed is None:
            raise ValueError(f"upsert not allowed for table: {table!r}")
        bad_cols = set(row.keys()) - allowed
        if bad_cols:
            raise ValueError(
                f"upsert into {table} with disallowed columns: {sorted(bad_cols)}"
            )
        if key not in allowed:
            raise ValueError(f"conflict key {key!r} not allowed for {table!r}")
