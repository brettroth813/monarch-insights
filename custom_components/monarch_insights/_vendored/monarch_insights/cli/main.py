...
def config_init(
    path: Path = typer.Option(
        None,
        help="Where to write the config. Defaults to the first writable DEFAULT_SEARCH_PATH.",
    ),
    filing_status: str = typer.Option(
        "single", help="Federal filing status: single | mfj | mfs | hoh"
    ),
    low_balance_floor: int = typer.Option(1500, help="Balance-forecast alert floor in USD"),
    notify_service: str = typer.Option(
        "",
        help="HA notify.* service for warn-tier alerts (e.g. notify.mobile_app_phone)",
    ),
    force: bool = typer.Option(False, "--force", help="Overwrite an existing config file."),
):
    """Auto-generate a minimal ``monarch_insights.yaml`` by pulling live Monarch data.

    Prereqs: ``auth login`` + ``sync full`` already run so the cache has accounts.

    The file only records things Monarch itself doesn't know (filing status, target
    allocation, alert thresholds) plus a flagged ``primary_checking_account_id``.
    Account display names, institutions, and balances are read directly from Monarch
    at runtime — this file stays small on purpose.
    """
    target_path = path or next((p for p in DEFAULT_CONFIG_PATHS if p.parent.is_dir()), None)
    if target_path is None:
        target_path = DEFAULT_CONFIG_PATHS[1]
        target_path.parent.mkdir(parents=True, exist_ok=True)

    if target_path.exists() and not force:
        console.print(f"[red]{target_path} already exists.[/] Pass --force to overwrite.")
        raise typer.Exit(1)
    ...
    if not raw_accounts:
        console.print(
            "[red]No accounts in cache.[/] Run `monarch-insights auth login` "
            "then `monarch-insights sync full` first."
        )
        raise typer.Exit(1)
    from monarch_insights.models import Account, AccountType, AccountSubtype

    accounts = [Account.model_validate(r) for r in raw_accounts]
    depository = [
        a for a in accounts if a.type == AccountType.DEPOSITORY and a.subtype == AccountSubtype.CHECKING
    ]

    if not depository:
        console.print(
            "[yellow]No checking accounts found; you'll need to set primary_checking_account_id manually.[/]"
        )
        primary = accounts[0] if accounts else None
    elif len(depository) == 1:
        primary = depository[0]
        console.print(f"[green]Primary checking:[/] {primary.display_name} ({primary.id})")
    else:
        table = Table(title="Which checking account pays your bills?")
        table.add_column("#", justify="right")
        table.add_column("Name")
        table.add_column("Balance", justify="right")
        table.add_column("ID", style="dim")
        for idx, acct in enumerate(depository, start=1):
            bal = f"${acct.current_balance:,.0f}" if acct.current_balance is not None else "—"
            table.add_row(str(idx), acct.display_name, bal, acct.id)
        console.print(table)
        pick = typer.prompt("Pick a number", type=int, default=1)
        primary = depository[max(1, min(pick, len(depository))) - 1]

    # Build the YAML payload by hand — a heredoc keeps it readable + lets us preserve
    # inline comments for the operator to tweak later.
    lines = [
        "# monarch_insights.yaml — auto-generated by `monarch-insights config init`.",
        "# Account names/balances are pulled live from Monarch; nothing about your",
        "# accounts is stored here.",
        "",
        f"filing_status: {filing_status}",
    ]
    if primary is not None:
        lines.append(f"primary_checking_account_id: {primary.id}")
    lines.extend(
        [
            "",
            "allocation_targets:",
            ...
            "",
            "alerts:",
            f"  low_balance_floor: {low_balance_floor}",
            "  concentration_threshold_pct: 15",
            "  price_move_threshold_pct: 5",
        ]
    )
    if notify_service:
        lines.append(f"  notify_service: {notify_service}")
    else:
        lines.append("  # notify_service: notify.mobile_app_your_phone")
    lines.extend(
        [
            "",
            "# watchlist:",
    ...
    console.print(f"[green]✔[/] wrote {target_path} (mode 0600)")
    console.print("Edit to taste, then re-run any command — the library reloads on each run.")


@config_app.command("show")
def config_show():
    """Print the parsed user config (sensitive fields are fine — file has no tokens)."""
    from monarch_insights.config import load

    cfg = load()
    if not cfg.is_configured:
        console.print(
            "[yellow]No config file found.[/] Run `monarch-insights config init` "
            "or create one at /config/monarch_insights.yaml."
        )
        raise typer.Exit(1)
    console.print(f"[dim]source:[/] {cfg.source_path}")
    console.print(f"[bold]filing_status:[/] {cfg.filing_status}")
    console.print(f"[bold]primary_checking_account_id:[/] {cfg.primary_checking_account_id}")
    console.print(f"[bold]allocation targets:[/] {len(cfg.allocation_targets)} buckets")
    for bucket, target in cfg.allocation_targets.items():
        console.print(f"  {bucket}: {target.target_pct}% (drift ±{target.drift_threshold_pct}%)")
