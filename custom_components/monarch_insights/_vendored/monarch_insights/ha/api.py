...
# Intentionally do NOT instantiate the FastAPI app at import time — constructing it
# opens SQLite files and runs schema DDL, which is an unexpected side effect for any
# caller that only imports this module to inspect it. Callers that want the ASGI app
# (uvicorn, tests) should invoke ``create_app()`` explicitly, e.g.::
#
#     uvicorn "monarch_insights.ha.api:create_app" --factory --host 0.0.0.0 --port 8765
